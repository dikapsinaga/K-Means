...
def likelihood(objects, selectedClass, discreteValues):
    likely = 1

    classCount = [object.classification for object in objects].count(selectedClass)

    # menghitung banyaknya fitur dalam class
    for i in range(0, len(discreteValues)):
        # menghitung banyak class yg dipilih
        count = 0
        for object in objects:
            if object.classification == selectedClass and object.discreteFeatures[i] == discreteValues[i]:
                count += 1
...
print(decision(data, ["kayu bakar", "plester"], [12, 3.5]))
